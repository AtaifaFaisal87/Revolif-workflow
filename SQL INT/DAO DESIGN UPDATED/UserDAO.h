...
//   exists                    SELECT EXISTS(SELECT 1 FROM users WHERE user_id=?)
...
public:
    ...
    // ---- Create ----
    [[nodiscard]] int64_t create(const UserRecord& user);

    // ---- Read ----
    [[nodiscard]] std::optional<UserRecord> findById(int64_t userId);
    [[nodiscard]] std::optional<UserRecord> findByUsername(const std::string& username);
    [[nodiscard]] std::optional<UserRecord> findByEmail(const std::string& email);
    [[nodiscard]] std::vector<UserRecord>   findAll();
    [[nodiscard]] std::vector<UserRecord>   findByRole(const std::string& role);
    [[nodiscard]] bool exists(int64_t userId);
    [[nodiscard]] bool existsByUsername(const std::string& username);
    [[nodiscard]] bool existsByEmail(const std::string& email);
    [[nodiscard]] uint64_t countAll();
    [[nodiscard]] uint64_t countByRole(const std::string& role);

    // ---- Update ----
    [[nodiscard]] bool update(const UserRecord& user);
    [[nodiscard]] bool updateLastLogin(int64_t userId, const std::string& loginTimestampIso);
    [[nodiscard]] bool updateStreaks(int64_t userId, int currentStreak, int bestStreak);
    [[nodiscard]] bool updateDisplayedAchievement(int64_t userId,
                                     std::optional<int64_t> achievementId);
    [[nodiscard]] bool setActive(int64_t userId, bool active);

    // ---- Delete ----
    [[nodiscard]] bool remove(int64_t userId);
};
