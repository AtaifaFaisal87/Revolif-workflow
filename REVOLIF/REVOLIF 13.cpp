...
void speak(const string &message)
{
    ...
    DeleteFileA(marker.c_str()); // clear any stale marker from a previous run
...
public:
    ...
    void displayProfile() const
    {
        ...
        cout << "LAST LOGIN       : ";
        if (lastLogin.toComparable() == 20000101)
            cout << "Never";
        else
            cout << lastLogin;
        cout << endl;
...
public:
...
#ifdef _WIN32
            // Stay on the loading screen only for as long as the TTS
            // engine genuinely takes to load/configure (the real,
            // noticeable delay). This is a real wait tied to real work
            // -- it ends the moment the marker appears, and is capped
            // so it can never hang if something goes wrong.
            {
                string marker = getTtsMarkerPath();
                int waited = 0;
                const int pollMs = 40;
                ...
                {
                    showLoadingStep("Starting voice assistant...", percent);
                    Sleep(pollMs);
                    waited += pollMs;
                    if (percent < 99)
                        percent++;
...
#endif
            showLoadingStep("Starting voice assistant...", 100);
            cout << "\n\n";
    ...
    void displayUserSummary(User *user)
    {
        ...
        cout << "LAST LOGIN        : ";
        if (user->getLastLogin().toComparable() == 20000101)
            cout << "Never";
        else
            cout << user->getLastLogin();
        cout << endl;
