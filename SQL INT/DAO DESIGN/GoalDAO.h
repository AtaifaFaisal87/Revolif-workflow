...
public:
    ...
    // ---- Create ----
    int64_t create(const GoalRecord& goal);

    // ---- Read ----
    std::optional<GoalRecord> findById(int64_t goalId);
    std::vector<GoalRecord>   findByUser(int64_t userId, const PageRequest& page = {});
    std::vector<GoalRecord>   findByUserAndStatus(int64_t userId, const std::string& status);
    std::vector<GoalRecord>   findByUserAndCategory(int64_t userId, int64_t categoryId);
    std::vector<GoalRecord>   findByUserAndTitle(int64_t userId, const std::string& titleFragment);
    std::vector<GoalRecord>   findByUserAndDeadline(int64_t userId, const std::string& deadlineDateIso);
    uint64_t                  countCompletedByUser(int64_t userId);

    // ---- Update ----
    bool update(const GoalRecord& goal);
    bool updateStatus(int64_t goalId, const std::string& status,
                       std::optional<std::string> completedAtIso);

    // ---- Delete ----
    bool remove(int64_t goalId);
